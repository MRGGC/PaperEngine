...
namespace Paper
{
	...
	#define PAPER_LOG(x) std::cout << "[Paper Audio]  " << x << std::endl
	...
	static ALenum GetOpenALFormat(uint32_t channels)
	{
		...
		PAPER_CORE_ASSERT(false, "Channel size is not supported!");
	...
	AudioSource Audio::LoadAudioSourceOgg(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			PAPER_LOG("File Info - " << filename << ":");
			PAPER_LOG("  Channels: " << channels);
			PAPER_LOG("  Sample Rate: " << sampleRate);
			PAPER_LOG("  Expected size: " << bufferSize);
		}
		...
		PAPER_CORE_ASSERT(bufferSize == size, "Buffer Size isn't equal to Size");

		if (s_DebugLog)
			PAPER_LOG("  Read " << size << " bytes");
		...
		if (alGetError() != AL_NO_ERROR)
			PAPER_LOG("Failed to setup sound source");
	...
	AudioSource Audio::LoadAudioSourceMP3(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			...
			PAPER_LOG("  Channels: " << channels);
			PAPER_LOG("  Sample Rate: " << sampleRate);
			PAPER_LOG("  Size: " << size << " bytes");

			auto [mins, secs] = result.GetLengthMinutesAndSeconds();
			PAPER_LOG("  Length: " << mins << "m" << secs << "s");
		}
	...
	static void PrintAudioDeviceInfo()
	{
		if (s_DebugLog)
		{
			PAPER_LOG("Audio Device Info:");
			PAPER_LOG("  Name: " << s_AudioDevice->DeviceName);
			PAPER_LOG("  Sample Rate: " << s_AudioDevice->Frequency);
			PAPER_LOG("  Max Sources: " << s_AudioDevice->SourcesMax);
			PAPER_LOG("    Mono: " << s_AudioDevice->NumMonoSources);
			PAPER_LOG("    Stereo: " << s_AudioDevice->NumStereoSources);
		}
	}
